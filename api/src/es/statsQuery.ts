...
export function buildStatsAggBody(opts: BuildStatsOptions = {}): Record<string, any> {
    const topCardsSize = Number.isFinite(opts.topCardsSize) ? Math.max(1, Math.min(50, opts.topCardsSize!)) : 10;
    ...
    return {
        ...
        aggs: {
            ...
            last_hour: {
                filter: { range: {createdAt: {gte: "now-1h", format: "strict_date_optional_time"}}},
            },
            last_day: {
                filter: { range: {createdAt: {gte: "now-24h", format: "strict_date_optional_time"}}},
            },
            last_week: {
                filter: { range: {createdAt: {gte: "now-7d", format: "strict_date_optional_time"}}},
            },
            avg_fraud_score: {
...
export function buildTrendsAggBody(opts: BuildTrendsOptions ): Record<string, any>{
    ...
    const histogramInterval =
        Number.isFinite(opts.histogramInterval) ? Math.max(0.01, Math.min(1, opts.histogramInterval!)): 0.1;
