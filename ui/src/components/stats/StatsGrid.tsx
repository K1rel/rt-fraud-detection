        ...
        <Tooltip>
            <TooltipTrigger asChild>
                <div className="flex items-center gap-2 cursor-default">
                    ...
                    <div className="hidden xl:block text-xs text-muted-foreground tabular-nums">{label}</div>
                </div>
            </TooltipTrigger>
...
export function StatsGrid({
    ...
    const { stats, trends, derived, loading, error, refreshMs } = data;
    ...
    const gridClass =
        layout === "wide" ? "grid gap-6 grid-cols-1 md:grid-cols-2" : "grid gap-4 grid-cols-1 sm:grid-cols-2";
    ...
    const avgScoreValue = typeof derived.avgFraudScore === "number" ? fmt.nf3.format(derived.avgFraudScore) : "—";
